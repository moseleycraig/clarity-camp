...
(define-public (stream-to
    ...
    (timeframe (tuple (start-block uint) (stop-block uint)))
    (payment-per-block uint)
  ) ;; let for data mapping variable: type
  ;; ;et blocks can be used to define some temporary variables to make the code easier to read inside of functions
  (let (
    (stream {
...
(define-read-only (calculate-block-delta
  ...
  (let (
    ...
    (current-block (block-height))  ;; Call block-height as a function
