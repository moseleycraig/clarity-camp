...
;; create a read-only function that returns a listing by ID as usual
...
(define-public (cancel-listing (listing-id uint) (nft-asset-contract <nft-trait>))
	(let (
		...
		(map-delete listings listing-id) as-contract (transfer-nft nft-asset-contract (get token-id listing) tx-sender maker))
	)
)
